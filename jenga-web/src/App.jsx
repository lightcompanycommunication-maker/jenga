...
export default function App() {
  ...
  const runJob = useCallback(async (job, lowData=false) => {
    ...
    try {
      ...
      setStreamText("");
      const parsed = (() => {
        // Stack-based JSON repair — handles truncated responses
        function repairTruncatedJson(s) {
          const stack=[];let inString=false,escaped=false,lastCharIdx=-1;
          for(let i=0;i<s.length;i++){const ch=s[i];if(!/\s/.test(ch))lastCharIdx=i;
        ...
        try{return JSON.parse(repairTruncatedJson(trimmed));}catch(e){
          throw new Error(`Format de réponse invalide — réessaie avec une description plus courte. (${e.message})`);
        }
      })();
